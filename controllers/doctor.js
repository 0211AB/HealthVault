...
exports.signupDoctor = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ ...saved_Doctor._doc, Message: "Doctor Sign Up Succesful" });
  } catch (e) {
    // console.log(e)
    if (e.code == 11000)
      return res
  ...
  try {
    ...
    if (!doctor) res.status(404).json({ Error: "Doctor not found" });
      ...
      if (isPasswordValid) {
        ...
        res
          .status(200)
          .json({ ...saved_Doctor._doc, Message: "Login Successful" });
      } else {
...
exports.updateDoctor = async (req, res) => {
  try {
    ...
    if (!doctor) res.status(404).json({ Error: "Invalid Credentials" });
      ...
      if (req.file != null) doctor.photo = req.file.filename;
      // console.log(doctor);
      const token = await doctor.generateAuthToken();
      const saved_doctor = await doctor.save();
      res
        .status(200)
        .json({ ...saved_doctor._doc, Message: "Doctor updated sucessfully" });
    }
  } catch (e) {
    if (e.code == 11000)
      return res
