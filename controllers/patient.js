...
exports.signupPatient = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ ...saved_patient._doc, Message: "Patient Sign Up Succesful" });
  } catch (e) {
    if (e.code == 11000)
      return res
        .status(400)
  ...
  try {
    ...
    if (!patient) res.status(404).json({ Error: "Patient not found" });
      ...
      if (isPasswordValid) {
        ...
        res
          .status(200)
          .json({ ...saved_patient._doc, Message: "Login Successful" });
      } else {
...
exports.updatePatient = async (req, res) => {
  try {
    ...
    if (!patient) res.status(404).json({ Error: "Invalid Credentials" });
      ...
      if (req.file != null) patient.photo = req.file.filename;
      // console.log(patient);
      const token = await patient.generateAuthToken();
      const saved_patient = await patient.save();
      res
