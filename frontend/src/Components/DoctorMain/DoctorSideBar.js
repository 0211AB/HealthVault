...
const DoctorSideBar = () => {
  ...
  useEffect(() => {
    ...
    const logoutF = async () => {
      const res = await fetch(`http://localhost:8000/api/doctor/logout`,
        {
          method: 'GET',
          headers: {
    ...
    <>
      ...
      <header className={navClasses}>
        ...
        <nav className="navbar">
          <Link to={`/doctor/${isOpen}`}> <i></i> Profile </Link>
          <Link to='/doctor/update'> <i></i> Update Profile </Link>
          <Link to='/doctor/history'> <i></i> Patient Details</Link>
          <Link to='/doctor/prescription'> <i></i> Create Prescription </Link>
          <Link to='/doctor/appointment'> <i></i> Appointments </Link>
