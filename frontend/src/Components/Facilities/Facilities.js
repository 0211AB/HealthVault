...
const Facilities = () => {
  ...
  useEffect(() => {
    ...
    setUrl(
      `https://maps.google.com/maps/embed/v1/place?q=${
        data?.data?.display_name || "Kolkata"
      }&key=AIzaSyB933ANy5iPjo3MpN9zNYztWeVcqy2KIY8&zoom=15`
    );
  ...
  const changeHandler = () => {
    setUrl(
      `https://maps.google.com/maps/embed/v1/search?q=hospitals+pharamacies+healthcare+health+urgentcare+medicalcollege+in+${
        location?.current?.value || data.data.display_name
